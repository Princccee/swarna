    ...
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Invoice-level sales summary with GST and payment status</p>
        </div>
      </div>
                ...
                {invoices.map((inv: any) => (
                  <tr key={inv.id} className="hover:bg-muted/50">
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_STYLES[inv.status] ?? 'bg-muted text-muted-foreground'}`}>
                        {inv.status}
                      </span>
                    </td>
                  </tr>
              ...
              {totals && invoices.length > 0 && (
                <tfoot className="bg-amber-50 border-t-2 border-amber-200">
                  <tr>
                    ...
                    <td className="px-4 py-3 text-right font-bold text-foreground">{fmt(totals.total)}</td>
