...
function TypeBadge({ type }: { type: OrderType }) {
  const m = TYPE_META[type] ?? { label: type, cls: 'bg-muted text-muted-foreground' };
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${m.cls}`}>
      {m.label}
    </span>
    ...
    <div className="p-6 space-y-5">
        ...
        <label className="flex items-center gap-2 text-sm cursor-pointer select-none">
          <span
            ...
            className={[
              ...
              dueSoon ? 'bg-amber-600' : 'bg-muted',
            ].join(' ')}
          >
            <span
              className={[
                'pointer-events-none inline-block h-4 w-4 rounded-full bg-card shadow transform transition-transform',
                dueSoon ? 'translate-x-4' : 'translate-x-0',
              ].join(' ')}
            />
          </span>
          <span className="text-muted-foreground">Due within 3 days</span>
        </label>
      </div>
      ...
      <div className="bg-card rounded-xl border overflow-hidden">
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="p-10 text-center text-muted-foreground/60 text-sm">Loading orders…</div>
          ) : (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      ...
                      {order.customer?.phone && (
                        <div className="text-xs text-muted-foreground/60">{order.customer.phone}</div>
                      )}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span
                        className={
                          order.balanceDue > 0 ? 'text-red-600 font-semibold' : 'text-muted-foreground'
                        }
                      >
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={page === 1}
              className="px-3 py-1 border rounded text-muted-foreground hover:bg-muted/50 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Prev
            </button>
            <button
